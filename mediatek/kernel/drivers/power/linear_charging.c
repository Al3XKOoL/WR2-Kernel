...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "TINNO_QUICK_CHARGING cv_voltage = %d\n\r", cv_voltage);  
    return cv_voltage;
}
...
PMU_STATUS do_jeita_state_machine(void)
{
    ...
    else if(BMT_status.temperature >= TEMP_POS_10_THRESHOLD)  
    {
        ...
        else
        {
            ...
            #ifdef HIGH_BATTERY_VOLTAGE_SUPPORT
            g_jeita_recharging_voltage = JEITA_TEMP_POS_10_TO_POS_45_RECHARGE_VOLTAGE; //4200;
            #else
...
static void set_jeita_charging_current(void)
{
	...
	if(BMT_status.charger_type == STANDARD_HOST)
		return;
#endif
    //BEGIN <High battery> <DATE2014801> <High battery> limi.zhang
    #if 0
	if(g_temp_status == TEMP_NEG_10_TO_POS_0)
    {
    ...
    #endif
	//END <High battery> <DATE2014801> <High battery> limi.zhang
}
