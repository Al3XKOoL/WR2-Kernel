...
my $PARTITION_DEFINE_C    = "$custom_out_prefix/kernel/partition_define_private.h"; #1
...
sub ProcessRawPartitionLayoutData
{
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
        ...
        if($partition_layout_process[$partition_idx]->{Partition_Name} eq "PGPT"){
			$PGPT_index=$partition_idx;
        }
        if($partition_layout_process[$partition_idx]->{Partition_Name} eq "SGPT"){
			$SGPT_index=$partition_idx;
        }
    }
    ...
    for ($partition_idx = 0 ; $partition_idx < @partition_layout_process ; $partition_idx++)
    {
    	...
    	if ($ArgList{EMMC_SUPPORT} eq "yes"){
	    	if($partition_idx > $PGPT_index && $partition_idx < $SGPT_index){
				$part->{Part_Idx}=$gpt_index++;
	    	}else{
				$part->{Part_Idx}=0xff;
	    	}
    	}
...
sub GenPartitionDefineC ()
{
    ...
    foreach my $part (@partition_layout_cooked)
    {
        ...
        if ($ArgList{EMMC_SUPPORT} eq "yes")
        {
            $temp = " EMMC, $part->{Part_Idx},$region_map{$part->{Region}}";
        }
        else
        {
