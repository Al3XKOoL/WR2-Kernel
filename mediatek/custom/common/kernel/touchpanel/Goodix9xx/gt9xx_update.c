...
#include "tpd_custom_gt9xx.h"
    ...
    static unsigned char *tp_fw_buff;
#endif
...
//begin add by lishengli 20140219
static unsigned char * get_fwbuff( struct i2c_client *client, u32 *fw_size )
{
    unsigned char *gtp_fw;
    u8 sensor_id = 0;
    s32 ret = 0;
    ...
    if (SUCCESS == ret)
    {
        ...
        switch(sensor_id)
        {
            case 0:
                ...
                *fw_size = sizeof(gtp_dijing_FW);
    	      break;
     
            case 1:
                gtp_fw = gtp_shenyue_FW;
                *fw_size = sizeof(gtp_shenyue_FW);				
                break;
    			
            defalt:
                gtp_fw = gtp_default_FW;
                *fw_size = sizeof(gtp_default_FW);
                break;
        }
    }
    else
    ...
    GTP_INFO("get_fwbuff lishengli666  : fw_size =  %d, sensor_id = %d",*fw_size, sensor_id);  
    return gtp_fw;
}
//end add by lishengli 20140219 
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head, u8 *path)
{
    ...
    else
    {
	 ...
	 tp_fw_buff = get_fwbuff(client, &fw_size);
	 GTP_INFO("gup_check_update_file lishengli666  : fw_size =  %d",fw_size);
        if (fw_size < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
        {
            GTP_INFO("[check_update_file]default firmware array is INVALID!");
            return FAIL;
        }
        memcpy(buf, &tp_fw_buff[0], FW_HEAD_LENGTH);
        ...
        {
            u16 temp;
            memcpy(buf, &tp_fw_buff[FW_HEAD_LENGTH + i], 2);
            temp = (buf[0]<<8) + buf[1];
            fw_checksum += temp;
        }
...
static u8 gup_load_section_file(u8 *buf, u16 offset, u16 length)
{
    ...
    {
        //tp_fw_buff = get_fwbuff(i2c_client_point);
        memcpy(buf, &tp_fw_buff[FW_HEAD_LENGTH + offset], length);
        return SUCCESS;
    }
#endif
    {
