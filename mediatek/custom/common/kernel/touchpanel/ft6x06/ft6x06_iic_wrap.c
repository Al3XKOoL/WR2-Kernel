...
#if defined(FT5436)
extern u8 is_5436_new_bootloader;
extern u8 is_5436_fwsize_30;
#endif
...
int ft5x0x_i2c_Write(char *writebuf, int writelen)
{
	...
	ret = i2c_transfer(g_pts->client, msg, 1);
	if (ret < 0)
		CTP_DBG("i2c write error.\n");
	return ret;
}
...
static ssize_t fts_bin_write_block_no_dma(uint8_t* pbt_buf, int dw_lenth)
{
	...
	packet_number = (dw_lenth) / FTS_PACKET_NO_DMA_LENGTH;
	buf[0] = 0xbf;
	buf[1] = 0x00;
	...
	if(is_5436_new_bootloader == BL_VERSION_LZ4 || is_5436_new_bootloader == BL_VERSION_Z7 )
	{
		for (i = 0; i<6; i++)
		{
			if (is_5436_new_bootloader  == BL_VERSION_Z7) 
			{
				temp = 0x7bfa + i;
			}
			else if(is_5436_new_bootloader == BL_VERSION_LZ4)
			{
				temp = 0x6ffa + i;
			}
