...
static int fwu_check_version(void)
{
	...
	retval = fwu->fn_ptr->read(fwu->rmi4_data,
				...
				sizeof(firmware_id));
	if (retval < 0) {
		CTP_DBG("Failed to read firmware ID (code %d).\n", retval);
		return retval;
	}
	firmware_id[3] = 0;

	CTP_DBG("Device firmware ID%d\n",
	...
	CTP_DBG("Device config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
		config_id[0], config_id[1], config_id[2], config_id[3]);

	//.img config id
	CTP_DBG(".img config ID 0x%02X, 0x%02X, 0x%02X, 0x%02X\n",
			fwu->config_data[0],
			fwu->config_data[1],
			fwu->config_data[2],
			fwu->config_data[3]);
...
//	return 0;
	//CTP_DBG("fwu_read_f34_flash_status = 0x%x\n", fwu->flash_control.data[0]);
	...
	for (block_num = 0; block_num < block_cnt; block_num++) {
		...
		if (block_num % progress == 0)
			CTP_DBG(
					...
					block_num, block_cnt);
	#endif
	  //  CTP_DBG("before write BLOCK_DATA_OFFSET");
		retval = fwu->fn_ptr->write(fwu->rmi4_data,
			fwu->f34_fd.data_base_addr + BLOCK_DATA_OFFSET,
			block_ptr,
			fwu->block_size);
		if (retval < 0) {
			CTP_DBG("%s: Failed to write block data (block %d)\n",
				__func__, block_num);
			return retval;
		}
...
static int fwu_write_bootloader_id(void)
{
	...
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			...
			fwu->bootloader_id,
			sizeof(fwu->bootloader_id));
	CTP_DBG("bootloader ID=%s\n",fwu->bootloader_id);
	CTP_DBG("f34_data_addr=%x\n",fwu->f34_fd.data_base_addr + BLOCK_DATA_OFFSET);
...
static int fwu_do_reflash(void)
{
	...
	CTP_DBG("%s: Erase all command written\n",
			__func__);
...
{   
	...
	CTP_DBG("%s: Start of reflash process\n", __func__);
...
int synaptics_fw_updater(unsigned char *fw_data)
{
	...
	image_configID = ConvertConfigID(fw_data+0xb100);
       CTP_DBG("touch panel: image_configID=%d, device_configID=%d", image_configID, device_configID);
	if(image_configID > device_configID){	
          //LINE <touch panel> <DATE2013821> <start upgrade> zhangxiaofei
		...
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			case SYNAPTICS_RMI4_F01:
				...
				if (status.flash_prog == 1) {
					CTP_DBG("%s: In flash prog mode, status = 0x%02x\n",
							__func__,
							status.status_code);
...
flash_prog_mode:
	...
	for (ii = 0; ii < rmi4_data->num_of_intr_regs; ii++) {
		if (rmi4_data->intr_mask[ii] != 0x00) {
			CTP_DBG("%s: Interrupt enable mask %d = 0x%02x\n",
...
static int synaptics_rmi4_fwu_init(struct i2c_client *client)
{
	...
	if (retval < 0) {
		CTP_DBG("%s: Failed to read PDT properties, assuming 0x00\n",
				__func__);
...
*/
	...
	if (retval < 0)
      ...
      CTP_DBG("query_base_addr=0x%x, data_base_addr=0x%x, addr_f34_flash_control=0x%x, bootloader_id=%s\n", 
