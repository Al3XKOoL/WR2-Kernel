...
static void ac_update(struct ac_data *ac_data)
{
    ...
    {         
        if ( (BMT_status.charger_type == NONSTANDARD_CHARGER)  || 
             ...
             //BEGIN <charging> <DATE2014716> <add tinno special charger> limi.zhang
             #if defined(TINNO_QUICK_CHARGING)
			                                                   ||
             (BMT_status.charger_type == APPLE_2_1A_CHARGER)   ||
             (BMT_status.charger_type == TINNO_1_5A_CHARGER)
             #endif
             //END <charging> <DATE2014716> <add tinno special charger> limi.zhang
             )
        {
            ac_data->AC_ONLINE = 1;        
...
//BEGIN <charging> <DATE2014726> <add tinno special charger> limi.zhang
#if defined(TINNO_QUICK_CHARGING)
#define MAX_CHARGING_PG 900000
#define MAX_CHARGING_CURRENT 1300
#define MAX_CHARGER_VOL 6000
#define CHARGER_OFFEST 150
kal_uint32 maxPgFlag = 0;
kal_int32 batteryAveragePg=0;
#endif
//END <charging> <DATE2014726> <add tinno special charger> limi.zhang
...
{ 
    ...
    battery_xlog_printk(BAT_LOG_CRTI, "bat_vol=%d, Vsense=%d, ICharging=%d, charger_vol=%d\n",
		                               bat_vol,Vsense,ICharging,charger_vol);
	...
	//BEGIN <charging> <DATE2014726> <add tinno special charger> limi.zhang
	#if defined(TINNO_QUICK_CHARGING)
	v = (charger_vol - Vsense) + CHARGER_OFFEST;
	pg = v*ICharging;
...
{
	char strbuf[30];
    sprintf(strbuf, "%d", charger_type);
    return sprintf(buf, "%s", strbuf);
}

static DEVICE_ATTR(charger_type_info, 0444, charger_type_show, NULL);
#endif
