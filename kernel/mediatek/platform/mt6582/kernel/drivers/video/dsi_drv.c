...
//BEGIN < lcd > <DATE20140523> <lcd blinking after SN > zhangxiaofei
DSI_STATUS DSI_Wait_VDO_Idle_Polling()
{
	long ret;
	unsigned int read_timeout_ms = 200;
	DSI_SetMode(0);
	while(DSI_REG->DSI_INTSTA.VM_DONE== 0) //clear
    {
        ///keep polling
    ...
    OUTREGBIT(DSI_INT_ENABLE_REG,DSI_REG->DSI_INTSTA,VM_DONE,1);

    return DSI_STATUS_OK;
}
//END < lcd > <DATE20140523> <lcd blinking after SN > zhangxiaofei
