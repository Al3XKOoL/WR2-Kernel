...
//Line added by mickal.ma
extern unsigned char ESD_CHECKING;
...
void DISP_WaitVSYNC(void)
{
	...
	if (wait_event_interruptible_timeout(vsync_wq, vsync_wq_flag, HZ/10) == 0)
	{
		printk("[DISP] Wait VSync timeout. early_suspend=%d\n", is_early_suspended);
			...
			if(ESD_CHECKING){
				for(id=0;id<=4;id++){
					if(captured_layer_config[id].layer_en){
						captured_layer_config[id].buff_idx = cached_layer_config[id].buff_idx;
						realtime_layer_config[id].buff_idx = captured_layer_config[id].buff_idx;
					}
				}
				printk("[DISP] VSync timeout caused by ESD Checking\n");			
			}
			//End added by mickal.ma
	}
